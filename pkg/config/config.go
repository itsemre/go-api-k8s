...
type Config struct {
	...
	ServerAddress        string   `mapstructure:"SERVER_ADDRESS" name:"server-address" long:"server-address" defaultValue:"0.0.0.0" help:"The address that the web server will be listening to"`
...
func BindCobraFlags(cmd *cobra.Command, v *viper.Viper, envPrefix string) error {
	...
	cmd.Flags().VisitAll(func(f *pflag.Flag) {
		// Environment variables don't dashes in their names, so bind them to their equivalent
